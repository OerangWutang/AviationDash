...
| Login throttle | Five consecutive failures per normalized username produce a 60-second `429`. State is in PostgreSQL and shared across API workers. Unknown usernames run a real verify against a fixed dummy Argon2 hash; stale unlocked throttle rows are swept after a quiet window. |
